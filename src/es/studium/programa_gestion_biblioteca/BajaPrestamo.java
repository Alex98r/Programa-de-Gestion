...
public class BajaPrestamo implements WindowListener, ActionListener {
	Frame ventana = new Frame("Baja Prestamo");
	Dialog dlgConfirmacion = new Dialog(ventana, "¿Segur@?", true);
	Dialog dlgMensaje = new Dialog(ventana, "Mensaje", true);

	Label lblElegir = new Label("Elegir el prestamo a borrar:");
	Label lblConfirmar = new Label("¿Seguro que quieres borrar el prestamo?");
	...
	BajaPrestamo() {
		...
		// Rellenar el Choice con los elementos de la tabla prestamos
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnCancelar)) {
			...
			if (choPrestamos.getSelectedIndex() != 0) {
				...
				lblConfirmar.setText("¿Seguro que quieres borrar el prestamo:  '" + choPrestamos.getSelectedItem() + "'?");
			...
			int resultado = conexion.eliminarPrestamo(tabla[0]); // prestamo --> DELETE
